...
class Game {
    constructor(initialState){
        if (initialState) this.state = initialState.map((row)=>[
                ...row
            ]);
        else this.state = [
            [
    ...
    start() {
        if (this.status === "idle") {
            this.status = "playing";
            const isEmpty = this.state.every((row)=>row.every((cell)=>cell === 0));
            if (isEmpty) {
                this._spawnTile();
                this._spawnTile();
            }
    ...
    moveRight() {
        this._processMove(()=>{
            this.state = this.state.map((row)=>this._slideAndMerge([
                    ...row
                ].reverse()).reverse());
        });
    }
    ...
    moveDown() {
        this._processMove(()=>{
            let transposed = this._transpose(this.state);
            transposed = transposed.map((row)=>this._slideAndMerge([
                    ...row
                ].reverse()).reverse());
            this.state = this._transpose(transposed);
        });
    ...
    _processMove(moveAction) {
        ...
        if (oldState !== newState) {
            this._spawnTile();
            this._checkWin();
            if (this.status !== "win") this._checkLose();
        }
    }
    _slideAndMerge(row) {
        let filtered = row.filter((val)=>val !== 0);
        for(let i = 0; i < filtered.length - 1; i++)if (filtered[i] !== 0 && filtered[i] === filtered[i + 1]) {
            filtered[i] *= 2;
    ...
    _checkWin() {
        for(let r = 0; r < 4; r++){
            for(let c = 0; c < 4; c++)if (this.state[r][c] === 2048) {
                this.status = "win";
                return;
            }
        }
    ...
    _checkLose() {
        ...
        for(let r = 0; r < 4; r++)for(let c = 0; c < 4; c++){
            ...
            if (c < 3 && val === this.state[r][c + 1]) return;
            if (r < 3 && val === this.state[r + 1][c]) return;
        }
        this.status = "lose";
    }
}
